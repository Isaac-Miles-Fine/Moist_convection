...
exp_name_short = 'forced-convection_lower_boundary_cr=1_sst=300_plots_full'
...
def diffusive_forcing(solver, state, dstatedt):
    ...
    T_forcing = -cooling_rate * y**2 * (solver.zs >= boundary_layer_top) 
    s_forcing = -(s-s_profile)*(solver.zs > 6000)/60 + T_forcing *(solver.zs >200)*(solver.zs <6000) * solver.cvd / T 
...
if run_model:
    ...
    print(noise)
    # centre_temp_original = T[15:16]
    # print(centre_temp_original)
    ...
    for i, tend in enumerate(tends):
        ...
        if rank == 0:
            ...
            print("Percentage done:", (100*solver.time)/run_time)
            print('Estimated time remaining:"', (run_time-solver.time)*(time.time() - t0)/60)
            ...
            u_kinetic_energy.append(solver.energy_kin()[0])
            w_kinetic_energy.append(solver.energy_kin()[1])
...
# plotting
    ...
    labels = ["entropy", "density", "water", "vapour", "T", "u", "w", "ice"]
...
plt.ylabel('Energy (Joules)')  # Label for the y-axis
plt.title('All energy Plots')
plt.legend()
...
plt.plot(X, u_kinetic_energy, color='m', label='Horizontal Kinetic Energy')
plt.legend()
plt.title('Horizontal Kintetic Energy')
plt.xlabel('Time (seconds)')  # Label for the x-axis
plt.ylabel('Energy (Joules)')
plt.legend(fontsize=14)
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Horizontal_KE.png'.format(exp_name_short))
plt.clf()
        
plt.plot(X, w_kinetic_energy, color='c', label='Vertical Kinetic Energy')
plt.legend()
plt.xlabel('Time (seconds)')  # Label for the x-axis
plt.ylabel('Energy (Joules)')
plt.title('Vertical Kinetic Energy')
plt.legend(fontsize=14)
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Vertical_KEp.png'.format(exp_name_short))
plt.clf()

plt.plot(X, u_kinetic_energy, color='m', label='Horizontal Kinetic Energy')
plt.plot(X, w_kinetic_energy, color='c', label='Vertical Kinetic Energy')
plt.title('Horizontal and Vertical Kinetic Energy')
plt.legend()
plt.xlabel('Time (seconds)')  # Label for the x-axis
plt.ylabel('Energy (Joules)')
plt.legend(fontsize=14)
plt.savefig('/Users/isaacmiles-fine/Google Drive/My Drive/scie30001/Energy_Plots/Kinetic_sep_com.png'.format(exp_name_short))
plt.clf()